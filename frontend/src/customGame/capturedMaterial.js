...
import { DRAGON_COST, POINT_COSTS, WIZARD_COST } from "../pieces/flat2dPieces";

// Dragon/Wizard aren't directly draftable (they only ever come from
// evolving a Knight/Bishop), so they have their own cost constants instead
// of a POINT_COSTS entry - and their own art key ("D"/"W", not the "R"/"B"
// slot they're actually stored as - see flat2dPieces.jsx). "kind" here is
// this module's own vocabulary: every letter POINT_COSTS already uses,
// plus these two synthetic ones.
const KIND_INFO = {
  ...
  A: { icon: "A", cost: POINT_COSTS.A },
  H: { icon: "H", cost: POINT_COSTS.H },
  C: { icon: "C", cost: POINT_COSTS.C },
  M: { icon: "M", cost: POINT_COSTS.M },
  DRAGON: { icon: "D", cost: DRAGON_COST },
  WIZARD: { icon: "W", cost: WIZARD_COST },
};
...
function heroCounts(snapshot, isWhite) {
  ...
  const s = snapshot || {};
  return {
    dragon: s[`${prefix}dragon_square`] ? 1 : 0,
    wizard: (s[`${prefix}wizard_squares`] || []).length,
    archer: (s[`${prefix}archer_squares`] || []).length,
...
// (A/H/C/M) DeckBuilder's own POINT_COSTS keys off, so cost lookup and
...
function capturedFromArmy(initialFen, initialSnapshot, currentFen, currentSnapshot, isWhite) {
  ...
  const capturedWizard = lost(initialHero.wizard, currentHero.wizard);
  ...
  // Dragon -> Rook slot, Wizard/Mirror -> Bishop slot, Archer/Hydra ->
  // Knight slot, Cyclops -> Pawn slot (see flat2dPieces.jsx's own comment
  // on which FEN type each hero piece is actually stored as).
  const capturedPlainR = Math.max(0, lost(initialBase.r, currentBase.r) - capturedDragon);
  const capturedPlainB = Math.max(0, lost(initialBase.b, currentBase.b) - capturedWizard - capturedMirror);
  ...
  const entries = [
    ...
    { kind: "DRAGON", count: capturedDragon },
    { kind: "WIZARD", count: capturedWizard },
    { kind: "A", count: capturedArcher },
...
export function computeCapturedMaterial(gameState) {
  ...
  const currentSnapshot = {
    white_dragon_square: gameState.white_dragon_square,
    black_dragon_square: gameState.black_dragon_square,
    white_wizard_squares: gameState.white_wizard_squares,
    black_wizard_squares: gameState.black_wizard_squares,
    white_archer_squares: gameState.white_archer_squares,
    black_archer_squares: gameState.black_archer_squares,
